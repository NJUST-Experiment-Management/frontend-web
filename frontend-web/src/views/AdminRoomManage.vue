			...
			<el-row :gutter="20" v-for="itemRow of deviceData" :key="itemRow" :span="5" :offset=itemRow*2
				...
				<el-card>
					<el-button :type="deviceStatusColor(device.deviceStatus)" v-for="device of itemRow " :key="device"
						style="width: 70px;text-align: center" @click="setDeviceState(device,device.deviceRow-1,device.deviceCol-1)"><i
				...
				this.$confirm('此操作修改机位状态, 是否继续?', '提示', {
					...
					request.post('/changeStatus/device',val).then(res => {
						...
						if(res.code==="0"){
							...
							if(this.deviceData[r][c]==='AVAILABLE')
							this.deviceData[r][c]='DISABLED'
							else
							this.deviceData[r][c]='AVAILABLE'
						}else{
							this.$message({
								type: "error",
								message: res.msg
