			...
			<el-form :model="form" ref="form" :rules="rules" style="padding: 0 5%  0  5% ;margin-top: 10px;">
				...
				<div>
					...
					<el-form-item label="上传学生名单:" required>
						<el-upload class="upload-demo" ref="upload" multiple action="" :limit="1" accept=".xls,.xlsx"
							:on-preview="handlePreview" :on-remove="handleRemove" :on-change="handleChange"
							:http-request="httpRequest" :file-list="fileList" :before-upload='handleBeforeUpload'
							:auto-upload="false">
							<template #trigger>
	...
	export default {
		...
		methods: {
			httpRequest(param) {
				...
				request.post('http://rap2api.taobao.org/app/mock/294824/addTest', fd, config).then(res => {
					if (res.code === '0') {
						this.$message({
							type: "success",
							message: "提交成功"
